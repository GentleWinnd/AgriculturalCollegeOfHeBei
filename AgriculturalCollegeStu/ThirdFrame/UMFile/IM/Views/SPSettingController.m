...
- (NSArray *)arraySectionAbout
{
    ...
    return @[
             ...
             [[SPSettingItem alloc] initWithTitle:[NSString stringWithFormat:@"清除缓存(%@)", sizeStr] selector:@selector(actionClearCache)],
             ];
}
...
{
    [[SPKitExample sharedInstance] exampleOpenEServiceConversationWithPersonId:kSPEServicePersonId fromNavigationController:self.navigationController];
}
...
{
    UIActionSheet *as = [[UIActionSheet alloc] initWithTitle:@"确认要清除缓存数据吗" delegate:self cancelButtonTitle:@"取消" destructiveButtonTitle:@"清除" otherButtonTitles:nil];
    [as setTag:kActionSheetTagClearCache];
    [as showInView:self.tabBarController.view];
}
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if (actionSheet.tag == kActionSheetTagLogout) {
        ...
        // 重启
        exit(0);
    } else if (actionSheet.tag == kActionSheetTagClearCache) {
        if (buttonIndex == 0) {
