...
- (void)_setWheelView {
    ...
    wheelCenterView = backView;
    wheelCenterView.backgroundColor = MainBackgroudColor_GrayAndWhite;
    ...
    for (int i=1; i<_scrollWidthProportion.count; i++) {
        ...
        UIView *lineM = [[UIView alloc] initWithFrame:CGRectMake(wheelCenterView.frame.origin.x+offsetY+self.picker_wheel_seperated_line.size.width*(i-1), wheelCenterView.frame.origin.y,1,CGRectGetHeight(self.bounds))];
        lineM.backgroundColor = RulesLineColor_LightGray;
...
- (void)_setTableViews:(NSUInteger)scroll {
    if (dataLoop) {
        ...
        for (int i=start; i<counts; i++) {
            ...
            IDJScrollComponent *scrollComponent=[[IDJScrollComponent alloc]initWithFrame:CGRectMake(0+x, 0, [[_scrollWidthProportion objectAtIndex:i]floatValue]*wheelCenterView.bounds.size.width, height) withViews:views];
            [views release];
...
- (void)_setSelectionArea {
    ...
    CGFloat selectionY=rowHeight*_selectionPosition+WHEEL_SPACE-10;
   
    UIView *lineOne = [[UIView alloc] initWithFrame:CGRectMake(0, selectionY, CGRectGetWidth(self.bounds), 1)];
    lineOne.backgroundColor = RulesLineColor_LightGray;
    [self addSubview:lineOne];
    
    UIView *lineTwo = [[UIView alloc] initWithFrame:CGRectMake(0,selectionY+rowHeight, CGRectGetWidth(self.bounds), 1)];
    lineTwo.backgroundColor = RulesLineColor_LightGray;
