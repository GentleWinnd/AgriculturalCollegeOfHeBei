...
//    [session.requestSerializer setValue:[self getAccessToken] forHTTPHeaderField:@"AccessToken"];
    ...
    [session POST:URL parameters:parametes progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        if (success) {
            success(responseObject);
        }
...
//    [session.requestSerializer setValue:[self getAccessToken] forHTTPHeaderField:@"AccessToken"];
     NSString *URL = [NSString stringWithFormat:@"%@&AccessToken=%@",URLString,[self getAccessToken]];
    [session GET:URL parameters:parametes progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        if (success) {
            success(responseObject);
        }
    } failure:^(NSURLSessionDataTask * _Nullable task, NSError * _Nonnull error) {
